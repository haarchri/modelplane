...
# resolved from a recipe. AICR has no `service` value for Nebius (see
# "Clouds AICR doesn't cover" in serving-stack-generation.md).
#
# Stand-in for functions/compose-serving-stack/function/stacks/nebius.py;
...
#
# Nebius' managed node images carry the kernel driver and container
# toolkit, so the operator installs neither and the driver sits at the
# default root - the same mode Modelplane takes on every cloud today.
# The network operator is not here for the same reason it is not in
# generated/aks.py: the cluster composition installs it when a pool
# asks for InfiniBand. (Fabric on Nebius also needs a named regional
# fabric domain fixed at provisioning - the cluster's business, not
# this list's; see "os and fabric as declared facts" in the design.)
#
...
# this list when AICR grows a `service` value for Nebius - the
